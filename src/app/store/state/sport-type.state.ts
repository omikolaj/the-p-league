...
export class SportTypeState {
  constructor(
    private scheduleAdminAsyncService: ScheduleAdministrationAsyncService
  ) {}
  ...
  static getSportTypesLeaguesPairs(
    state: SportTypeStateModel,
    leagueState: LeagueStateModel
  ) {
    const pairs: Required<SportTypesLeaguesPairs[]> = [];
  ...
  fetchFailed(
    ctx: StateContext<SportTypeStateModel>,
    action: Sports.FetchAllSportTypesFailed
  ) {}
  ...
  add(ctx: StateContext<SportTypeStateModel>, action: Sports.AddSportType) {
    ctx.setState(
      produce((draft: SportTypeStateModel) => {
        ...
        draft.entities[action.newSportType.id].leagues =
          draft.entities[action.newSportType.id].leagues || [];
    ...
    ctx.setState(
      produce((draft: SportTypeStateModel) => {
        action.payload.forEach(pair => {
          ...
          draft.entities[pair.sportTypeID].leagues = (
            draft.entities[pair.sportTypeID].leagues || []
          ).concat([pair.ids]);
        });
      })
  ...
  update(
    ...
    ctx.setState(
      produce((draft: SportTypeStateModel) => {
        draft.entities[action.updatedSportType.id] = updateEntity(
          action.updatedSportType,
          draft.entities[action.updatedSportType.id]
        );
      })
    );
  }
    ...
    ctx.setState(
      produce((draft: SportTypeStateModel) => {
        action.deleteIDs.forEach(deleteID => {
          const index = draft.entities[action.sportTypeID].leagues.indexOf(
            deleteID
          );
          if (index !== -1) {
