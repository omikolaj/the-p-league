...
export function handleError(err, snackBar) {
	if (err.error instanceof ErrorEvent) {
		snackBar.openSnackBarFromComponent(`An error occured: ${err.error.message}`, 'Dismiss', SnackBarEvent.Error);
	} else {
		if (err.status === 500 || err.status === 501 || err.status === 504) {
			snackBar.openSnackBarFromComponent('Request to the server failed', 'Dismiss', SnackBarEvent.Error);
		}
		...
		if (ErrorCodes.includes(backendErrorKey)) {
			...
			if (err.status === 401) {
				...
				snackBar.openSnackBarFromComponent(`${err.error[errorKey].errors[0].errorMessage}`, 'Dismiss', SnackBarEvent.Error);
			} else {
				snackBar.openSnackBarFromComponent(`${err.error[errorKey][0]}`, 'Dismiss', SnackBarEvent.Error);
			}
		}
	}
