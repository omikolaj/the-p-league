...
export class PreOrderService extends MerchandiseService {
	...
	constructor(http: HttpClient, snackBar: SnackBarService, eventBus: EventBusService, router: Router) {
	...
	private preOrderGearItemAsync(preOrderForm: PreOrderForm): Observable<PreOrderForm> {
		this.loadingSubject.next(true);
		return this.http
			.post<PreOrderForm>(`${this.merchandiseUrl}/${preOrderForm.gearItemId}/pre-order`, JSON.stringify(preOrderForm), this.headers)
			.pipe(
				tap(() => this.loadingSubject.next(false)),
				catchError((err) => {
					this.loadingSubject.next(false);
					this.snackBarService.openSnackBarFromComponent('Error occured submitting the pre-order', 'Dismiss', SnackBarEvent.Error);
					return of(null);
				})
			);
	}
